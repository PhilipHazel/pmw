...
switch(size)
  ...
  default:
  if (size > UserFontSizes)
    P("/S%d", size - UserFontSizes + 1);
  else P("/s%d", size + 1);
  break;
  }
}
...
static void
write_headfoot(headstr *p, const char *directive, const char *pre,
  const char *post)
{
for (; p != NULL; p = p->next)
  {
  if (p->drawing == NULL)  /* Text heading */
    ...
    {
    /* Drawing not supported: there will be a warning caused by the existence
    of a drawing tree. */
    }
  }
}
...
for (usint movt = 0; movt < movement_count; movt++)
  ...
  for (int stave = 0; stave <= m->laststave; stave++)
    ...
    BOOL wasnote;
    BOOL incue = FALSE;
    BOOL overbeamset = FALSE;
    barstr **barvector, *b;
    b_accentmovestr *accentmove = NULL;
...
(void)note_total;  // Not currently used
    ...
    while (b != NULL)
      ...
      switch (b->type)
        ...
        case b_barnum:
        b_barnumstr *bn = (b_barnumstr *)b;
        P("[barnumber");
        if (!bn->flag) P(" off"); else write_move(bn->x, bn->y);
        P("] ");
        break;

        case b_beamacc:
        case b_beamrit:
        b_beamaccritstr *ar = (b_beamaccritstr *)b;
        P("[beam%s", (b->type == b_beamacc)? "acc":"rit");
        if (ar->value != accrit)
          {
          P(" %d", ar->value);
          accrit = ar->value;
        ...
        case b_beammove:
        P("[beammove %s] ", sff(((b_intvaluestr *)b)->value));
        break;

        case b_beamslope:
        P("[beamslope %s] ", sff(((b_intvaluestr *)b)->value));
        break;
        ...
        case b_bowing:
        P("[bowing %s] ", ((b_bowingstr *)b)->value? "above":"below");
        break;
        ...
        case b_dotright:
        dotright = ((b_dotrightstr *)b)->value;
        break;
        ...
        case b_ensure:
        P("[ensure %s] ", sff(((b_ensurestr *)b)->value));
        break;

        case b_footnote:
        write_headfoot(&(((b_footnotestr *)b)->h), "footnote", "\n[", "]");
        break;

        /* Each occurrence has only one flag set */

        case b_justify:
        uint8_t j = ((b_justifystr *)b)->value;
        P("[justify %c", ((j & just_add) == 0)? '-' : '+');
        switch(j & ~just_add)
          {
          case just_top: P("top"); break;
          case just_bottom: P("bottom"); break;
          case just_left: P("left"); break;
          case just_right: P("right"); break;
          }
        P("] ");
        break;
        ...
        case b_linegap:
        case b_slurgap:
        b_slurgapstr *gap = (b_slurgapstr *)b;
        P("[%sgap", (b->type == b_linegap)? "line" : "slur");
        if (gap->id != 0) P("/=%c", gap->id);
        if (gap->width != 4000) P("/w%s", sff(gap->width));
        if (gap->hfraction != -1) P("/h%s", sff(gap->hfraction));
        write_move(gap->xadjust, 0);
        if (gap->gaptext != NULL)
          {
          write_pmw_string(gap->gaptext, "/", font_rm);
          if (gap->textsize != 0) write_text_size(gap->textsize);
          if ((gap->textflags & text_boxrounded) != 0) P("/rbox");
            else if ((gap->textflags & text_boxed) != 0) P("/box");
          if ((gap->textflags & text_ringed) != 0) P("/ring");
          write_move(gap->textx, gap->texty);
          }
        P("] ");
        break;
        ...
        case b_notes:
        P("[notes %s] ", ((b_notesstr *)b)->value? "on" : "off");
        break;

        case b_ns:
        b_nsstr *ns = (b_nsstr *)b;
        P("[ns");
        for (int i = 0; i < NOTETYPE_COUNT; i++) P(" %s", sff(ns->ns[i]));
        P("] ");
        break;

        case b_nsm:
        P("[ns *%s] ", sff(((b_nsmstr *)b)->value));
        break;

        case b_olevel:
        P("[olevel %s] ", (((b_uolevelstr *)b)->value == FIXED_UNSET)?
          "*" : sff(((b_uolevelstr *)b)->value));
        break;

        case b_olhere:
        P("[olhere %s] ", sff(((b_uolevelstr *)b)->value));
        break;
        ...
        case b_page:
        b_pagestr *pgs = (b_pagestr *)b;
        P("\n[page %s%d] ", pgs->relative? "+" : "", pgs->value);
        break;

        case b_pagebotmargin:
        P("[bottommargin %s] ", sff(((b_intvaluestr *)b)->value));
        break;

        case b_pagetopmargin:
        P("[topmargin %s] ", sff(((b_intvaluestr *)b)->value));
        break;
        ...
        /* NB the argument for [transpose] is in semitones, relative to the
        current transposition, but the value in the item is in quarter tones,
        and is absolute. */

        case b_transpose:
        int tpa = ((b_transposestr *)b)->transpose;
        int tpr = tpa - transpose;
        transpose = tpa;
        P("[transpose %d] ", tpr/2);
        break;
        ...
        case b_tripsw:
        P("[tuplets %s] ", ((b_tripswstr *)b)->value? "on":"off");
        break;

        case b_ulevel:
        P("[ulevel %s] ", (((b_uolevelstr *)b)->value == FIXED_UNSET)?
          "*" : sff(((b_uolevelstr *)b)->value));
        break;

        case b_ulhere:
        P("[ulhere %s] ", sff(((b_uolevelstr *)b)->value));
        break;
        ...
        case b_zerocopy:
        P("[copyzero %s] ", sff(((b_zerocopystr *)b)->value));
        break;
        ...
        if (addsize) write_text_size(txt->size);
        ...
        case b_accentmove:
        if (accentmove == NULL) accentmove = (b_accentmovestr *)b;
        break;
        ...
        if (dotright != 0)
          {
          if (dotright != 5000) P("%s", sff(dotright));
          P(">");
          dotright = 0;
          }
        ...
        /* Accents. To get the accent number for handling a movement, we
        compute the accent number by shifting. As this is rare, there's no
        point in doing anything fancier. */

        while (acflags != 0)
          {
          b_accentmovestr *am = accentmove;
          ...
          for (accent *a = accent_chars; a->flag > 255; a++)
            ...
            if ((acflags & a->flag) != 0)
              ...
              if (am != NULL)
                {
                int accent_number = 0;
                uint32_t abits = a->flag;
                while (abits <= af_staccato)
                  {
                  accent_number++;
                  abits <<= 1;
                  }
                for (;;)
                  ...
                  if (am->accent == accent_number)
                    ...
                    cp = format_move(am->x, am->y, cp);
                    break;
                    }
                  am = (b_accentmovestr *)am->next;
                  if (am->type != b_accentmove) break;
                  }
                }
              acflags &= ~a->flag;
              break;
              }
            }    /* Loop searching for this accent */
          }      /* Loop through all accent flags */
        ...
        case b_draw:  /* Warning will have been given in header processing */
        break;

        case b_midichange:
        error(ERR202, "changing MIDI parameters");
        break;
        }
